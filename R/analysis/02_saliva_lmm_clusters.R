# =============================================================================
# Saliva — linear mixed model + temporal clusters (Discovery cohort).
#
# Baseline (t = -1) z-normalisation of the saliva proteome, then a linear mixed
...
# navmix (K = 6, seed 42).
#
# Input:  data/saliva_npx_data.rda, data/prot.label.saliva.rda, data-raw/clinical_data.xlsx
# Output: data/exerome.dat.saliva.rda, data/prot.label.saliva.rda, data/res.saliva.linear.rda
# =============================================================================
suppressMessages({
...
load(here("data", "saliva_npx_data.rda"));   npx.data   <- saliva_npx_data
...
exerome.dat.saliva <- exerome.dat %>%
  dplyr::mutate(time_label = as.numeric(as.character(time))) %>% dplyr::select(-time)
save(exerome.dat.saliva, file = here("data", "exerome.dat.saliva.rda"))
prot.label.saliva <- prot.label
save(prot.label.saliva, file = here("data", "prot.label.saliva.rda"))

registerDoParallel(cores = max(1, detectCores() - 1))
res.saliva.linear <- mixed_anova_parallel(exerome.dat, "t.factor", unique(prot.label$Assay),
                                          "+ (1|subject)", covariates = c("age", "vo2_max"))
stopImplicitCluster()
res.saliva.linear <- res.saliva.linear %>%
...
res.saliva.linear <- as.data.table(res.saliva.linear) %>% dplyr::rename(Assay = outcome) %>%
  dplyr::distinct(Assay, .keep_all = TRUE)

# navmix temporal clustering of the exercise-regulated proteins
cl.data <- res.saliva.linear %>% dplyr::filter(fdr.aov < .05) %>%
...
fit <- navmix(as.matrix(dplyr::select(cl.data, starts_with("zscore."))),
              K = 6, plot = FALSE, select_K = FALSE, reorder_traits = FALSE)
cl.data$cluster <- fit$fit$z

# navmix labels are arbitrary, so give each recomputed cluster the number of the
# original clusters (Supplementary Table 2, sheet B) that its proteins most overlap.
pub_cl <- as.data.frame(suppressWarnings(readxl::read_xlsx(
  here("data-raw", "supplementary_table_2.xlsx"), sheet = "B")))
...
res.saliva.linear <- res.saliva.linear %>% left_join(tmp.cl, by = "Assay") %>% arrange(fdr.aov)
save(res.saliva.linear, file = here("data", "res.saliva.linear.rda"))
cat(sprintf("Saliva: %d proteins, %d exercise-regulated (fdr<0.05). Cluster sizes:\n",
            nrow(res.saliva.linear), sum(res.saliva.linear$fdr.aov < 0.05, na.rm = TRUE)))
print(table(res.saliva.linear$cluster))
